...
`ZZFwUpdate` needs firmware with FWUP protocol support, and USB interrupt/ISO
support requires the matched firmware 2.9 and `zzusbhw.device` 2.2 pair;
...
| Network benchmark | `ZZNetBench` | `C:` | Raw SANA-II throughput benchmark (RX/RXD/TX/BUS modes). |
...
`ZZNetBench` measures raw SANA-II throughput through `ZZ9000Net.device`
and the MMIO bus constants that bound it:

```text
ZZNetBench RX 5 8     ; raw receive (staging fallback path)
ZZNetBench RXD 5      ; negotiated-hook receive (direct drain path)
ZZNetBench TX 5 1500  ; transmit blast — verify on a receiver (below)
ZZNetBench BUS 2      ; 32-bit MMIO read-rate constant
```

Measurement procedure (hold it constant across runs, or the numbers are
not comparable):

- Environment: screen blanked or a fixed low-depth mode (the ZZ9000's
  own RTG refresh shares the Zorro bus with every measurement), network
  stack stopped, quiet machine. Discard one warm-up run; report the
  median of at least three with their spread.
- Validity: an RX run counts only when the Overrun delta printed by the
  tool is ~0, and a sender-side rate from the peer belongs beside every
  RX number. A TX number is reportable only beside a matching
  receiver-side count on the peer — there is no FPGA TX counter to
  cross-check locally.
- Attribution: run `ZZNetStats` MONITOR alongside the first saturation
  blast and record `FirmwareRXPauseSent`, `FirmwareRXDropped`, and
  backlog state — this decides whether the card pauses the wire or
  drops under pressure.
- TCP layer: AmiSpeedTest under the same environment rules, including
  one run against a peer with TCP timestamps and window scaling
  disabled so the receive-coalescing path is actually exercised.

Record every result with the environment conditions, the BUS constant,
and the installed AmiNetXDuo and firmware versions —
`docs/plans/2026-09-20-1201-perf-zz9000net-throughput-plan.md` (U7)
