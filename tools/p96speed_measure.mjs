...
// P96SPEED_REFERENCE to comma-separated baseline exports from the same
...
for (const [index, name] of operations.entries()) {
  ...
  const ratio = sample.get(name) / reference;
  allFloor = Math.min(allFloor, ratio);
  // GUI timings vary independently of the RTG line path (SizeWindow ranged
  // from 122 to 48148 op/s on three identical baseline runs).
  if (index < 16) floor = Math.min(floor, ratio);
}
