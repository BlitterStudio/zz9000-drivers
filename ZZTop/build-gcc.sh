...
# mhi/build.sh and ahi/driver/build.sh: ZZ9000_SDK override, else a sibling
# checkout as-is, else clone/reuse the SDK at the pinned sdk/SDK_REF. Runs
# on the host before the docker re-exec below; inside the container the
# staged copy is already present.
script_dir=$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)
sdk_src=${ZZ9000_SDK:-}
if [ "$staged" -eq 1 ]; then
  # Second-stage (re-exec'd) invocation: the host leg already staged the
  # headers into ZZTop/zz9k-headers (validated below); never clone here.
  sdk_src=
else
