...
static inline int zzusb_interrupt_abort_after_retire(
    int armed, int arm_in_progress)
{
    return armed && !arm_in_progress;
}

static inline int zzusb_interrupt_abort_reply_ready(
    int abort_waiting, int armed)
{
    return abort_waiting && !armed;
}
